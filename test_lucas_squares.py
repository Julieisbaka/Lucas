...
class LucasSquareTests(unittest.TestCase):
                ...
                with self.subTest(layout=layout, count=count):
                    ...
                    for i, first in enumerate(squares):
                        for second in squares[i + 1:]:
                            overlap = (first.x < second.x + second.size and
                                       second.x < first.x + first.size and
                                       first.y < second.y + second.size and
    ...
    def test_auto_finds_best_count_within_limit(self):
        for layout in ("turning", "fit"):
            ...
            self.assertEqual(selected, min(
                candidates, key=lambda squares: (*layout_score(squares), -len(squares))))
            self.assertLessEqual(layout_score(selected)[0],
                                 layout_score(candidates[-1])[0])
    ...
    def test_turning_squares_share_an_edge_with_preceding_square(self):
        ...
        for previous, current in zip(squares, squares[1:]):
            horizontal_contact = (
                previous.x + previous.size == current.x or
                current.x + current.size == previous.x
            ) and max(previous.y, current.y) < min(
                previous.y + previous.size, current.y + current.size)
            vertical_contact = (
                previous.y + previous.size == current.y or
                current.y + current.size == previous.y

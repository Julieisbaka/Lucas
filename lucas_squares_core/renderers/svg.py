...
def render_svg(
    ...
    placement = page_placement(
        squares, page_width_units, page_height_units, margin * svg_units_per_inch
    )
    ...
    if alignment == "seamless":
        ...
        for x, y, filler_width, filler_height in filler_rectangles(squares):
            lines.append(
                f'<rect x="{x}" y="{y}" width="{filler_width}" height="{filler_height}"/>'
            )
        lines.append("</g>")
    lines.append(
        f'<g id="lucas" fill="none" stroke="black" stroke-width="{1.5 / scale:.12g}">'
    )

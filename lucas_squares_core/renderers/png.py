...
def render_png(squares: list[Square], output: Path, labels: bool, alignment: str,
               page_width: float, page_height: float, margin: float, dpi: int) -> None:
    ...
    for square in squares:
        ...
        if labels and side >= 15:
            font_size = max(1, round(min(side * 0.45, side * 1.35 / len(str(square.size)),
                                        dpi * 0.32)))
            try:
